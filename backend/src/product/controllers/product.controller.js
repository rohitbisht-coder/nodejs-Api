...
export const getAllProducts = async (req, res, next) => {
  ...
  try {
    ...
    const allProducts = await getAllProductsRepo(page);
    if (allProducts) {
      res.status(200).json({ success: true, allProducts });
    } else {
      return next(new ErrorHandler(400, "No more products  found!"));
    }
  } catch (err) {
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    if (reviewToBeDeleted.user == req.user._id) {
     ...
     reviews.forEach((rev) => {
      ...
      return next(new ErrorHandler(400, "Sorry You can remove only your review!"));
    }
  } catch (error) {
    return next(new ErrorHandler(500, error));
  }
